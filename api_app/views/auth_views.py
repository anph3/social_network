...
class AuthView(ViewSet):
    ...
    def create_token(self):
        ...
        access_token = self.jwt_encode(
            data = token, 
            key = vs.TOKEN['public_key']
        )
        refresh_token = self.jwt_encode(
            data = token, 
            key = vs.TOKEN['private_key']
        )
    ...
    def set_token_redis(self, a_token, r_token, data):
        ...
        cache.set(
            a_token, r_token, 
            timeout=vs.TOKEN['tls_access_token']
        )
        cache.set(
            r_token, data,
            timeout=vs.TOKEN['tls_refresh_token']
        )
